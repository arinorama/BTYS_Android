...
import com.besiktasshipyard.mobile.btys.busEvents.onGetReportList;
...
import com.besiktasshipyard.mobile.btys.fragments.reports.ReportListRecyclerViewAdapter;
 ...
 * {@link ReportListFragment.OnListFragmentInteractionListener} interface
 * to handle interaction events.
 * Use the {@link ReportListFragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class ReportListFragment extends Fragment implements SearchView.OnQueryTextListener, MenuItem.OnActionExpandListener{
    private Context context;

    private String TITLE = "Raporlar";

    private boolean _loadDataFromDB = true;
    private ReportListFragment.OnListFragmentInteractionListener mListener;

    private Context _context = null;

    private RecyclerView recyclerView;
    private ReportListRecyclerViewAdapter _adapter;

    ReportListData _ReportListData;
    ReportListFragment.ReportListFragmentState _fragmentState;
    ...
    public static ReportListFragment newInstance(boolean loadDataFromDB, Context context) {
    ...
    private void saveFragmentState(){
        ...
        _fragmentState = new ReportListFragment.ReportListFragmentState(_filterText, DataCache.getInstance().get_reportListData());
    }
...
//            _cachedRawData = _datacache.get_reportListData();
...
//            ((MainActivity) getActivity()).showLoadingPanel();
            ...
            boolean _getOnlySelfReports = false;
            Reports _reports = Reports.getInstance(context, recyclerView);
            _reports.getReportList(new onGetReportList(), _getOnlySelfReports);
        }
    }
     ...
     */
    @Subscribe
    public void onEvent(onGetReportList event){
        //gelen event te error varsa isle
        if (event.getError() != null)
        {
//            ApplicationHelpers.getInstance(getContext()).handleError("UserFragment: hata: onGetReportList - " + event.getError().toString());
            ApplicationHelpers.getInstance(_context).handleError(
                    new ApplicationErrorData(ApplicationErrorData.ApplicationErrorType.EVENT_DATA_ERROR, event.getError().toString(), this.getClass().getName() + " hata: onGetReportList")
            );
            return;
        }
        ...
        {
//            ApplicationHelpers.getInstance(getContext()).handleError("UserFragment: hata: onGetReportList - DATA yok");
            ApplicationHelpers.getInstance(_context).handleError(
                    new ApplicationErrorData(ApplicationErrorData.ApplicationErrorType.EVENT_DATA_ERROR, "", this.getClass().getName() + " hata: onGetReportList")
            );
            return;
        }
        ...
        //ReportList datasini gosterir
        showReportListData(event.getData());
        showRecordCount();
     ...
     * @param data
     */
    private void showReportListData(Object data){
        ...
        JSONArray _jaReportList = (JSONArray) data;
        _ReportListData = new ReportListData(_jaReportList);
        _adapter = new ReportListRecyclerViewAdapter(_ReportListData.ITEMS, mListener);
    ...
    private void showRecordCount(){
        ...
        _tvAppBarInfo.setText("Toplam Rapor Sayisi: " + _recordCount);
    ...
    public void onAttach(Context context) {
        ...
        _context = context;
        if (context instanceof ReportListFragment.OnListFragmentInteractionListener) {
            mListener = (ReportListFragment.OnListFragmentInteractionListener) context;
        } else {
...
//endregion
    ...
    public interface OnListFragmentInteractionListener {
        void onListFragmentInteraction(ReportListData.ReportListItem item);
    }

    private class ReportListFragmentState{
        private Object _ReportListDataState;
        private String _filterText;

        public ReportListFragmentState(String filterText, Object ReportListDataState){
